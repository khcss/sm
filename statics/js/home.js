        ...
        'BuyPopedom':function(o){
            ...
            if($that.attr("data-id")){
                if (confirm('您确认购买此条数据播放权限吗？')) {
                    $.ajax({
                        url: maccms.path + '/user/ajax_buy_popedom.html?id=' + $that.attr("data-id") + '&mid=' + $that.attr("data-mid") + '&sid=' + $that.attr("data-sid") + '&nid=' + $that.attr("data-nid") + '&type=' + $that.attr("data-type"),
                        cache: false,
                        dataType: 'json',
                        success: function ($r) {
                            $that.addClass('disabled');
...
jQuery(document).ready(function() {
    jQuery('.pianyi a').hover(function() {
        jQuery(this).animate({
            paddingLeft: "10px"//鼠标悬停距离左边间距
        }, 300);
    }, function() {
        jQuery(this).animate({
            paddingLeft: "0"//鼠标离开后设置距离左边多远
